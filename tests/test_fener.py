...
TO_SNAKE_CASE_REGEX = re.compile(
    r"(?<!^)(?=[A-Z])")  # type: typing.Pattern[str]
...
def compare_with_expected_file(
    ...
    actual_file_path = test_directory_path / "actual_output/201" / file_name
    expected_file_path = test_directory_path / "expected_output/201" / file_name
...
def assert_equality_of_actual_and_expected_output(  # pylint: disable=invalid-name
    ...
    results = [
        (file_names[i], compare_with_expected_file(
            test_directory_path, file_names[i], expected_error[i]))
        for i in range(len(file_names))
    ]
    assert results == [(file_name, True) for file_name in file_names], [
        ('File output %s may exceed the maximum expected error %1.3f.' %
         (file_names[i], expected_error[i]))
        for i in range(len(file_names))
    ]
...
class TestFener:
    class TestIntegration:
        ...
        def clean_up(
            ...
            actual_output_path = list()
            actual_output_path = test_directory_path / "actual_output/201"
            if actual_output_path.exists():
                shutil.rmtree(actual_output_path)
            workdir_path = test_directory_path / "workDir/201"
        ...
        def test_simple(
            ...
            run(f"python3 ./src/fener.py {test_directory_path}/201.fnr -geo -shoeBox -daylight -grid -lightSch -therm -thermComf -glareSimpl")
